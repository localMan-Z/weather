...
import { phaseFunctions } from "./GSAP.js";
import { location } from "./location.js";
import { futureForecast } from "./futureForecast.js";

const { assignColor } = colors();
const {
  after,
  form,
  inputOne,
...
const { generateFutureForecast, updateFutureForecastElements } =
  futureForecast();
const { generateLocation } = location();
let { phaseOne, phaseTwo, phaseThree, alternatePhaseOneTimeline } =
  phaseFunctions();
console.log("a");
...
async function queryImports() {
  try {
    let locationData = await phaseOne();
    return locationData;
  } catch (error) {
    console.error(error);
  }
...
async function search(value, random, nOne, nTwo) {
  ...
  if (random) {
    ...
    const { locationA, time, currentTemperature, description } = locationObject;
    city = locationA;
    if (locationArray) {
...
function updateDisplay(
  ...
  let temperaturePassed = temperaturDe;
  phaseThree(temperaturePassed);
}
